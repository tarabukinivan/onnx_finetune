...
# ==== CONFIG ====
DATA_DIR = "datasets19_57_combi"         # <-- Ваш путь к исходному .csv (с колонками Class, NewFileName)
DATA_CSV = "datasets19_57_combi/labels_nv400.csv"      # <-- Папка с изображениями
IMG_SIZE = 380
BATCH_SIZE = 8
EPOCHS = 40
LR = 1e-4
SEED = 42
MODEL_SAVE = "best_b4_balanced_real.pth"
...
train_transform = A.Compose([
    ...
    A.CoarseDropout(max_holes=7, min_height=8, min_width=8, max_height=32, max_width=32, p=0.25),
...
def main():
    ...
    # Для BEN_OTH апсемплим до 200, для INF до 250
    ...
    # ==== 3. Class weights ====
    class_counts = train_df['Class'].value_counts().reindex(TRAIN_CLASSES)
    weights = (max(class_counts) / class_counts).values
